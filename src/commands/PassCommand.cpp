...
bool PassCommand::noErrorsExist(ICommands *base, const int &clientSocket, Server *server, const std::string &command) const
{
	...
	if (base->isParameterEmpty(command) || getClientPass().empty())
	{
		std::string errMsg = ":" ERR_NEEDMOREPARAMS
			BOLDRED " ERROR :" BOLDWHITE "No password given" RESET "\r\n";
	...
	{
		std::string errMsg = ":" BOLDRED "ERROR :" BOLDWHITE "Already authenticated" RESET "\r\n";
		sendResponse(clientSocket, errMsg);
		return (false);
	}
	...
	{
		std::string errMsg = ":" ERR_PASSWDMISMATCH
			BOLDRED " ERROR :" BOLDWHITE "Invalid password" RESET "\r\n";
		sendResponse(clientSocket, errMsg);
		return (false);
	}
	return (true);
...
{
	(void)client;
	if (base->getParameters(command).size() == 0)
	{
		std::string errMsg = ":" ERR_NEEDMOREPARAMS
			BOLDRED " ERROR :" BOLDWHITE "No password given" RESET "\r\n";
		sendResponse(clientSocket, errMsg);
